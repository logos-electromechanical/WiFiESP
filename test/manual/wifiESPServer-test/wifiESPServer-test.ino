...
void loop() {
  ...
  WiFiESPServer *servers[CLIENT_COUNT];
  uint8_t clientCtr = 0;
  uint8_t serverCtr = 0;
  ...
  switch(selection) {
    case 0:
      ...
      Serial.println(F("Select the ESP Server function to run"));
      Serial.println(F("20) Create a new server"));
      Serial.println(F("21) Start a server already created"));
      Serial.println(F("22) Get a connected client"));
      Serial.println(F("23) Get server status (general)"));
      Serial.println(F("24) Get server status (particular mux)"));
      Serial.println(F("25) Write a byte to all connected clients"));
      Serial.println(F("26) Write a buffer to all connected clients"));
      Serial.println(F("27) Stop a given server"));
      Serial.println(F("28) Return current number of servers"));
      break;
    case 1:
    ...
    case 19:
      ...
      break;
    case 20:      
      Serial.println(F("Creating a new server..."));
      Serial.println(F("Enter desired port"));
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      port = Serial.parseInt();   
      if (serverCtr < CLIENT_COUNT) {
        servers[serverCtr] = new WiFiESPServer(port);
        serverCtr++;
      } else {
        Serial.println(F("Maximum number of clients reached"));
      }
      break;
    case 21:
      Serial.println(F("Start a previously created server"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      mux = Serial.parseInt();
      servers[mux]->begin();
      break;
    case 22:
      Serial.println(F("Get a client object from a server"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      mux = Serial.parseInt();
      if (clientCtr < CLIENT_COUNT) {
        clients[clientCtr] = servers[mux]->available(&mode);
        Serial.println(mode);
        clientCtr++;
      } else {
        Serial.println(F("Maximum number of clients reached"));
      ...
      break;
    case 23:
      Serial.println(F("Get the status of a particular server (general)"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      mux = Serial.parseInt();
      Serial.println(servers[mux]->status());
      break;
    case 24:
      Serial.println(F("Get the status of a particular server on a particular socket"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      item = Serial.parseInt();
      ...
      while (!Serial.available());
      mux = Serial.parseInt();      
      Serial.println(servers[item]->status(mux));
      break;
    case 25:
      Serial.println(F("Write a byte to all connected clients on a given server"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      item = Serial.parseInt();
      Serial.println(F("Enter character to send"));
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      c = Serial.read();
      Serial.println(servers[item]->write((uint8_t)c));
      break;
    case 26:
      Serial.println(F("Write a buffer to all connected clients on a given server"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      item = Serial.parseInt();
      Serial.println(F("Enter a buffer to send"));
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      inString1 = Serial.readString();
      Serial.println(servers[item]->write(inString1.c_str(), inString1.length()));
      break;
    case 27:
      Serial.println(F("Stop a given server"));
      Serial.print(F("Enter server to use (up to ")); Serial.print(serverCtr); Serial.println(')');
      while(Serial.available()) Serial.read();
      while (!Serial.available());
      item = Serial.parseInt();
      ...
      break;
    case 28:
      Serial.print(F("There are ")); Serial.print(serverCtr); Serial.println(F(" servers"));
      break;
